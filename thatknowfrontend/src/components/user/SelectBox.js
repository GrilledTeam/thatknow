...
export default function SelectBox({ areaSelects, onAreaSelectsChange }) {
    ...
    return (
        <div>
            <select className="stateList" value={selectedState} onChange={handleStateChange}>
                <option>-- state --</option>
                {state.map((item) => (
                    <option key={item.state} value={item.state}>{item.state}</option>
                ))}
            </select>
            <select className="cityList" value={selectedCity} onChange={handleCityChange}>
                <option>-- city --</option>
                {city.map((item) => (
                    <option key={item.city} value={item.city}>{item.city}</option>
                ))}
            </select>
            <select className="townList" value={selectedTown} onChange={handleTownChange}>
                <option>-- town --</option>
                {town.map((item) => (
